...
лежат прямо в [GitHub Release v2.8.1](../../releases/tag/v2.8.1):

| Файл | Для чего |
|---|---|
| `chatvpn_2.8.1_amd64.deb` | Debian/Ubuntu, 64-bit ПК |
| `chatvpn_2.8.1_arm64.deb` | Debian/Ubuntu, ARM (Pi и т.д.) |
| `chatvpn-2.8.1-1.x86_64.rpm` | Fedora/RHEL/openSUSE, 64-bit ПК |
| `chatvpn-2.8.1-1.aarch64.rpm` | Fedora/RHEL, ARM |
| `chatvpn-2.8.1-linux-amd64.tar.gz` | Arch, Steam Deck, Alpine, всё прочее |
| `chatvpn-2.8.1-linux-arm64.tar.gz` | то же, ARM |

```bash
# .deb
sudo apt install ./chatvpn_2.8.1_amd64.deb

# .rpm
sudo dnf install ./chatvpn-2.8.1-1.x86_64.rpm

# .tar.gz
tar xzf chatvpn-2.8.1-linux-amd64.tar.gz
cd chatvpn-2.8.1-linux-amd64 && sudo ./install.sh
```
...
- **Bazzite / Fedora Silverblue / Kinoite / Bluefin** — используйте `rpm-ostree install` со скачанным `.rpm`-файлом и перезагружайтесь:
  ```bash
  sudo rpm-ostree install ./chatvpn-2.8.1-1.x86_64.rpm
